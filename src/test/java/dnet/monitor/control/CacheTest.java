...
import dnet.monitor.control.config.ConfigCache;
...
/*	
	public static class HangListener implements RPCListener {
		...
		@Override
		public synchronized void received(String id, String s) {
			called = true;
			this.notify();
	...
	private static CommandServer cache;
	...
	public static void makeCache() throws IOException{
		Map<String, String> metadata = new HashMap<String, String>();
		List<CommandHandler> handlers = new LinkedList<>();
		handlers.add(new ConfigCache());
		
		cache = new CommandServer("testS"+System.currentTimeMillis(), makeConnection(), metadata, handlers); 
		cache.start();
	}
